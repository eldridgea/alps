...
func newServer(logger Logger, options *Options) (*Server, error) {
	...
	s.Sessions.prefetchInboxOnLogin = options.PrefetchInboxOnLogin
	s.Sessions.prefetchInboxLimit = options.PrefetchInboxLimit
...
type Options struct {
	...
	PrefetchInboxOnLogin bool          // Proactively cache INBOX messages after login (default: false)
	PrefetchInboxLimit   int           // Max INBOX messages to prefetch when enabled (default: 100)
